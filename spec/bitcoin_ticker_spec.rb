...
module BitcoinTicker
  describe BitcoinTicker do
    ...
    before do
      ...
      expect(ENV).to receive(:[]).with("NEO_PRICE_THRESHOLD").and_return "5"
      ...
      expect(price_checker).to receive(:current_price).with(:neo) { current_neo_price }
      ...
      expect(price_saver).to receive(:read).with(:neo) { previous_neo_price }
